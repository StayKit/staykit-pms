...
async function main() {
  ...
  const priya = await prisma.user.create({
    data: { ownerId: owner.id, name: "Priya R.", email: owner.email, phone: owner.phone, role: "OWNER" },
  });
  ...
  const rtDeluxe = await prisma.roomType.create({
    data: { propertyId: p1.id, name: "Deluxe Cottage", color: "#1B5E5A", baseRate: 6300_00, maxOccupancy: 3, sortOrder: 0 },
  });
  const rtStandard = await prisma.roomType.create({
    data: { propertyId: p1.id, name: "Standard Room", color: "#3D5A80", baseRate: 4200_00, maxOccupancy: 2, sortOrder: 1 },
  });
  const rtSuite = await prisma.roomType.create({
    data: { propertyId: p1.id, name: "Family Suite", color: "#E07A5F", baseRate: 7000_00, maxOccupancy: 4, sortOrder: 2 },
  });
  ...
  const rtHouseboat = await prisma.roomType.create({
    data: { propertyId: p2.id, name: "Houseboat", color: "#3D5A80", baseRate: 9500_00, maxOccupancy: 4 },
  });
  for (let i = 1; i <= 5; i++) {
    await prisma.room.create({
      data: { propertyId: p2.id, roomTypeId: rtHouseboat.id, name: `Houseboat ${i}`, number: String(i) },
    });
  }

  // Property scopes: Rakesh→p1 only; Anjali→p1 limited.
  await prisma.propertyScope.create({
    data: { userId: rakesh.id, propertyId: p1.id, permissions: "bookings:write,payments:refund,reports:read" },
  });
  await prisma.propertyScope.create({
  ...
  // ── Guests ──
  const guestDefs = [
    { key: "g1", name: "Sameer Khan", phone: "+919800014782", email: "sameer.k@gmail.com", city: "Bengaluru" },
    { key: "g2", name: "Anika Mehta", phone: "+919800088301", email: "anika.mehta@outlook.com", city: "Mumbai" },
    { key: "g3", name: "Rohan Iyer", phone: "+919900030217", email: "rohan@iyer.in", city: "Chennai" },
    { key: "g4", name: "Priyanka Joshi", phone: "+919800071109", email: "p.joshi@gmail.com", city: "Pune" },
    { key: "g5", name: "Daniel Müller", phone: "+4915245670000", email: "d.mueller@web.de", city: "Berlin", foreign: true, nationality: "German" },
    { key: "g6", name: "Vikram Singh", phone: "+919800041023", email: "vik.singh@yahoo.in", city: "Delhi" },
    { key: "g7", name: "Meera Krishnan", phone: "+919900080921", email: "meera.k@gmail.com", city: "Kochi" },
    { key: "g8", name: "Arjun Reddy", phone: "+919800055619", email: "arjun.r@hotmail.com", city: "Hyderabad" },
    { key: "g9", name: "Catherine Wong", phone: "+6591230000", email: "cwong@gmail.com", city: "Singapore", foreign: true, nationality: "Singaporean" },
  ] as const;
  ...
  const bookingDefs = [
    { ref: "SK-CO2401", room: "r-101", guest: "g3", start: -4, end: -1, src: "direct", total: 18900, paid: 18900, status: "CHECKED_OUT" },
    { ref: "SK-CO2402", room: "r-201", guest: "g4", start: -3, end: 0, src: "phone", total: 11400, paid: 11400, status: "CHECKED_IN", adults: 2, children: 1 },
    { ref: "SK-CO2403", room: "r-103", guest: "g1", start: 0, end: 3, src: "direct", total: 18900, paid: 9450, status: "CONFIRMED" },
    { ref: "SK-CO2404", room: "r-202", guest: "g2", start: 0, end: 2, src: "airbnb", total: 8400, paid: 8400, status: "CONFIRMED" },
    { ref: "SK-CO2405", room: "r-302", guest: "g5", start: 0, end: 5, src: "booking", total: 39750, paid: 0, status: "CONFIRMED" },
    { ref: "SK-CO2406", room: "r-301", guest: "g6", start: -1, end: 2, src: "direct", total: 24300, paid: 24300, status: "CHECKED_IN", adults: 2, children: 2 },
    { ref: "SK-CO2407", room: "r-102", guest: "g7", start: 1, end: 4, src: "phone", total: 16800, paid: 0, status: "TENTATIVE" },
    { ref: "SK-CO2408", room: "r-101", guest: "g8", start: 2, end: 6, src: "mmt", total: 22400, paid: 11200, status: "CONFIRMED" },
    { ref: "SK-CO2409", room: "r-201", guest: "g9", start: 3, end: 7, src: "booking", total: 14000, paid: 14000, status: "CONFIRMED", adults: 1 },
    { ref: "SK-CO2410", room: "r-202", guest: "g4", start: 4, end: 8, src: "whatsapp", total: 16800, paid: 0, status: "CONFIRMED" },
    { ref: "SK-CO2411", room: "r-103", guest: "g6", start: 4, end: 6, src: "direct", total: 12600, paid: 6300, status: "CONFIRMED", children: 1 },
    { ref: "SK-CO2412", room: "r-302", guest: "g1", start: 6, end: 10, src: "instagram", total: 31800, paid: 31800, status: "CONFIRMED" },
    { ref: "SK-CO2413", room: "r-301", guest: "g3", start: 3, end: 5, src: "direct", total: 16200, paid: 0, status: "CONFIRMED" },
  ] as const;
  ...
  for (const b of bookingDefs) {
    ...
    const booking = await prisma.booking.create({
      data: {
        ...
        checkedInAt: b.status === "CHECKED_IN" || b.status === "CHECKED_OUT" ? addDays(checkIn, 0) : null,
        checkedOutAt: b.status === "CHECKED_OUT" ? checkOut : null,
        guests: { create: { guestId: guestByKey[b.guest], isPrimary: true } },
        rooms: {
          create: nights.map((date) => ({ roomId: roomByKey[b.room], date, rateApplied: ratePerNight })),
        },
      },
    });
  ...
  const templates: {
    ...
    { channel: "SMS", triggerKey: "BOOKING_CONFIRMED", name: "Booking confirmation", body: "Hi {{guest.name}}, your booking {{booking.ref}} at {{property.name}} is confirmed for {{booking.checkIn|date}}.", dlt: "DLT_BK_CONF" },
    { channel: "EMAIL", triggerKey: "BOOKING_CONFIRMED", name: "Booking confirmation", subject: "Your stay at {{property.name}} is confirmed", body: "Dear {{guest.name}},\n\nYour booking {{booking.ref}} is confirmed.\nCheck-in: {{booking.checkIn|date}} at {{property.checkInTime}}." },
    { channel: "WHATSAPP", triggerKey: "BOOKING_CONFIRMED", name: "Booking confirmation", body: "Namaste {{guest.name}}! Booking {{booking.ref}} confirmed. See you on {{booking.checkIn|date}}.", wa: "booking_confirmed_v1" },
    { channel: "SMS", triggerKey: "PAYMENT_LINK_SENT", name: "Payment link", body: "{{guest.name}}, please pay {{amount.due|inr}} for {{booking.ref}}: {{paymentLink.url}}", dlt: "DLT_PAY_LINK" },
    { channel: "WHATSAPP", triggerKey: "PAYMENT_LINK_SENT", name: "Payment link", body: "Pay {{amount.due|inr}} securely for booking {{booking.ref}}: {{paymentLink.url}}", wa: "payment_link_v1" },
    { channel: "SMS", triggerKey: "PAYMENT_RECEIVED", name: "Payment received", body: "Thank you {{guest.name}}! We received your payment for {{booking.ref}}.", dlt: "DLT_PAY_RCVD" },
    { channel: "EMAIL", triggerKey: "PAYMENT_RECEIVED", name: "Payment received", subject: "Payment received — {{booking.ref}}", body: "We have received your payment. Your GST invoice is attached." },
    { channel: "WHATSAPP", triggerKey: "PRE_ARRIVAL_24H", name: "Check-in reminder", body: "See you tomorrow, {{guest.name}}! Check-in from {{property.checkInTime}} at {{property.name}}.", wa: "pre_arrival_v1" },
    { channel: "EMAIL", triggerKey: "POST_CHECKOUT_THANKS", name: "Post-stay thank you", subject: "Thank you for staying with us", body: "Dear {{guest.name}}, thank you for staying at {{property.name}}. We hope to host you again!" },
    { channel: "WHATSAPP", triggerKey: "POST_CHECKOUT_THANKS", name: "Post-stay thank you", body: "Thanks for staying with us, {{guest.name}}! A small review would mean a lot.", wa: "post_stay_v1" },
    { channel: "SMS", triggerKey: "CANCELLED", name: "Cancellation notice", body: "Your booking {{booking.ref}} has been cancelled. Refund (if any) will be processed shortly.", dlt: "DLT_CANCEL" },
    { channel: "EMAIL", triggerKey: "CANCELLED", name: "Cancellation notice", subject: "Booking {{booking.ref}} cancelled", body: "Your booking has been cancelled as requested." },
  ];
  ...
  if (preArrival) {
    await prisma.notificationAutomation.create({
      data: { ownerId: owner.id, triggerKey: "PRE_ARRIVAL_24H", templateId: preArrival.id, delayMinutes: -1440 },
    });
  }

  // ── Activity feed (AuditLog) ──
  const activity = [
    { actorType: "USER", actorName: "Rakesh", action: "CHECKED_IN", summary: "checked in Anika Mehta — Bamboo (202)", min: 18 },
    { actorType: "MCP", actorName: "Claude (AI)", action: "PAYMENT_LINK_SENT", summary: "sent payment link to Daniel Müller", min: 52 },
    { actorType: "USER", actorName: "Priya", action: "BOOKING_CREATED", summary: "created booking SK-CO2403 for Sameer Khan", min: 89 },
    { actorType: "SYSTEM", actorName: "System", action: "PAYMENT_CAPTURED", summary: "received ₹ 11,200 from Arjun Reddy", min: 130 },
    { actorType: "USER", actorName: "Anjali", action: "ROOM_STATUS", summary: "marked Cardamom (102) as dirty", min: 166 },
    { actorType: "USER", actorName: "Rakesh", action: "BOOKING_MODIFIED", summary: "extended stay for Rohan Iyer — Coffee Suite", min: 600 },
  ];
  ...
  const client = await prisma.mcpOAuthClient.create({
    data: {
      ...
      scopes: "bookings:read,bookings:write,payments:read,notifications:send,properties:read,reports:read,bookings:cancel,payments:refund",
    },
  });
  ...
  const mcpActions = [
    { tool: "send_notification", args: "to: Daniel Müller · template: payment_link", min: 52, status: "OK" },
