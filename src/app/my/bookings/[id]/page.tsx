...
export default async function GuestBookingDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const b = await prisma.booking.findFirst({
    ...
    include: { property: true, rooms: { include: { room: true } }, paymentLinks: { orderBy: { createdAt: "desc" } } },
  });
  if (!b) notFound();
      ...
      <div style={{ maxWidth: 460, margin: "0 auto", padding: "24px 16px" }}>
        ...
        <div style={{ background: "var(--surface)", border: "1px solid var(--line)", borderRadius: 16, overflow: "hidden", boxShadow: "var(--shadow-1)" }}>
          <div style={{ padding: "18px 18px 14px", borderBottom: "1px solid var(--line)" }}>
            ...
            <div style={{ fontWeight: 600, fontSize: 16, marginTop: 2 }}>{room ? `${room.name} · Room ${room.number}` : b.property.name}</div>
          ...
          <div style={{ padding: 18 }}>
            {due > 0 && (
              <a
                ...
                style={{ background: "var(--accent-soft)", borderRadius: 12, padding: 14, display: "flex", alignItems: "center", gap: 10 }}
              >
            ...
            <div style={{ marginTop: 8, padding: 12, borderRadius: 10, background: "var(--surface)", border: "1px solid var(--line)", fontSize: 12.5, lineHeight: 1.7 }}>
              <div><b>Check-in</b> · {shortDate(b.checkIn)}, {b.property.checkInTime}</div>
              <div><b>Check-out</b> · {shortDate(b.checkOut)}, {b.property.checkOutTime}</div>
              <div><b>Guests</b> · {b.adults} adults{b.children ? `, ${b.children} children` : ""}</div>
              <div><b>Total</b> · {inr(b.totalAmount)} ({inr(b.amountPaid)} paid)</div>
            </div>
