    ...
    <div className="page" style={{ paddingTop: 16 }}>
      ...
      <div className="kpi-grid">
        {cards.map((c) => (
          <div key={c.label} className="kpi" style={{ cursor: "default" }}>
            ...
            <div className="delta">Occupancy {c.k.occupancyPct}% · ADR {inr(c.k.adrPaise)}</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 20, marginTop: 24 }}>
        <div className="card card-padded">
          ...
          <div className="sub text-muted text-sm" style={{ marginTop: 2 }}>Plain definitions in tooltips</div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16, marginTop: 22 }}>
            <Metric label="Occupancy" value={`${monthK.occupancyPct}%`} hint="Room-nights sold ÷ available" />
            <Metric label="ADR" value={inr(monthK.adrPaise)} hint="Average price per room sold" />
            <Metric label="RevPAR" value={inr(monthK.revparPaise)} hint="Revenue per available room" />
            <Metric label="Room-nights sold" value={String(monthK.roomNightsSold)} hint="Across all properties" />
            <Metric label="Available" value={String(monthK.roomNightsAvailable)} hint="Room-nights on offer" />
            <Metric label="Room revenue" value={inr(monthK.roomRevenuePaise)} hint="Sum of nightly rates" />
          </div>
        </div>
            ...
            {mix.map((s) => (
              <div key={s.name} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderBottom: "1px solid var(--line)" }}>
                ...
                <div style={{ flex: "0 0 120px", background: "var(--surface-2)", height: 6, borderRadius: 3, overflow: "hidden" }}>
                  <div style={{ background: s.color, width: `${Math.min(100, s.pct * 2.5)}%`, height: "100%" }} />
                </div>
      ...
      <div className="checklist-card" style={{ marginTop: 24 }}>
        ...
        <div className="text">
          ...
          <div className="sub">5% GST on rooms ≤ ₹ 7,500/night · 18% above (SAC 996311). Email a copy to your CA.</div>
        </div>
...
function Metric({ label, value, hint }: { label: string; value: string; hint: string }) {
  return (
    <div title={hint}>
      <div className="text-xs text-muted" style={{ fontWeight: 550 }}>{label}</div>
      <div style={{ fontSize: 22, fontWeight: 600, letterSpacing: "-0.02em", marginTop: 4 }}>{value}</div>
      <div className="text-xs text-muted" style={{ marginTop: 2 }}>{hint}</div>
    </div>
  );
}
