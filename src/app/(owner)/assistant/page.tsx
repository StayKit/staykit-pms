          ...
          <h2 style={{ fontSize: 22, display: "flex", alignItems: "center", gap: 10 }}>
            ...
            {token ? (
              <span className="pill pill-checkedin">
                <Icon name="shield-check" className="icon-sm" /> Connected
              </span>
            ) : (
              <span className="pill pill-tentative">
                <Icon name="alert" className="icon-sm" /> Not connected
              </span>
            )}
          </h2>
          <div className="sub">
            Let an AI assistant run your homestay from inside Claude.ai — securely.
          </div>
        </div>
        ...
        <div className="card">
          ...
          <div style={{ padding: "0 20px 16px" }}>
            <div className="field" style={{ marginTop: 8 }}>
              ...
              <div className="hint">
                In Claude.ai → Customize → Connectors → Add custom connector, paste this URL.
              </div>
            </div>
            ...
            {token && (
              <div className="field" style={{ marginTop: 12 }}>
                ...
                >
                  <div className="avatar purple" style={{ width: 32, height: 32, fontSize: 12 }}>
                    C
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 550, fontSize: 13.5 }}>{token.client.clientName}</div>
                    <div className="text-xs text-muted">
                      Last used {token.lastUsedAt ? rel(token.lastUsedAt) : "—"} ·{" "}
                      {token.scopes.split(",").length} scopes
                    </div>
                  </div>
                  <button className="btn btn-sm btn-ghost" style={{ color: "var(--st-unpaid)" }}>
                    Revoke
                  </button>
                </div>
              </div>
          ...
          <div className="card-header" style={{ borderTop: "1px solid var(--line)" }}>
            ...
            <div className="sub" style={{ marginLeft: "auto" }}>
              {TOOL_CATALOG.length} tools · RBAC-enforced
            </div>
          </div>
          <table className="tbl">
            <thead>
            ...
            <tbody>
              {TOOL_CATALOG.map((t) => (
                <tr key={t.name} style={{ cursor: "default" }}>
                  ...
                  <td>
                    <span className="channel-chip">{t.scope}</span>
                  </td>
                  <td>
                    {t.requiresApproval ? (
                      <span className="pill pill-tentative">
              ...
              <div key={a.id} style={{ padding: 12, borderBottom: "1px solid var(--line)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <div style={{ marginLeft: "auto" }}>
                    {a.status === "OK" ? (
                      <span className="pill pill-checkedin">
                        <Icon name="check" className="icon-sm" /> OK
                      </span>
                    ) : (
                      <span className="pill pill-tentative">
                        <Icon name="clock" className="icon-sm" /> {a.status}
                      </span>
                    )}
                  </div>
                </div>
                ...
                <div
                  style={{ fontSize: 11.5, color: "var(--muted-2)", marginLeft: 36, marginTop: 2 }}
                >
                  {rel(a.createdAt)}
                </div>
              </div>
