...
describe("MCP route — unauthenticated negotiation", () => {
  ...
  it("returns a parse error for invalid JSON", async () => {
    const res = await POST(
      new Request("http://localhost:3000/mcp", { method: "POST", body: "{bad" }),
    );
    const body = await res.json();
...
describe("MCP route — authorized via the dev fallback", () => {
  ...
  it("rejects unknown tools and unknown methods", async () => {
    expect(
      (
        await (
          await rpc({ jsonrpc: "2.0", id: 6, method: "tools/call", params: { name: "ghost" } })
        ).json()
      ).error.code,
    ).toBe(-32602);
    expect(
      (await (await rpc({ jsonrpc: "2.0", id: 7, method: "frobnicate" })).json()).error.code,
    ).toBe(-32601);
  });
...
describe("MCP route — auth enforcement", () => {
  ...
  it("records DENIED when a tool is called outside the token's scope", async () => {
    ...
    expect(body.result.isError).toBe(true);
    const audit = await prisma.mcpAuditEntry.findFirst({
      where: { tool: "create_booking", status: "DENIED" },
    });
