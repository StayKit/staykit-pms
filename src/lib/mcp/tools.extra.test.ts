...
const ALL = [
  "bookings:read", "bookings:write", "bookings:cancel", "payments:read", "payments:refund",
  "properties:read", "properties:write", "reports:read", "notifications:send",
];
...
describe("tool argument branches", () => {
  it("list_bookings handles both empty and fully-specified filters", async () => {
    await createBooking({
      ...
      guest: { name: "S", phone: "+919812300000" }, nightlyRatePaise: 6300_00,
    });
    const all = (await getTool("list_bookings")!.run({}, ctx())) as unknown[];
    expect(all.length).toBe(1);
    const filtered = (await getTool("list_bookings")!.run(
      { propertyId: fx.property.id, status: "CONFIRMED", from: ymd(addDays(today(), -1)), to: ymd(addDays(today(), 30)) },
      ctx(),
    )) as unknown[];
